    ...
    var UI = /** @class */ (function () {
        ...
        UI.prototype.init = function () {
            if (!this.canvas.parentNode.classList.contains("molmil_UI_container")) {
                var container = molmil_dep.dcE("span");
                container.classList.add("molmil_UI_container");
            ...
            cont.appendChild(icon);
            cont.menu = cont.pushNode("div");
            cont.menu.style.display = "none";
            cont.menu.className = "molmil_UI_RM";
            (this.canvas ? this.canvas.parentNode : document.body).appendChild(cont);
        };
        ;
        UI.prototype.deleteEntry = function (entry) {
        };
        ;
        UI.prototype.displayEntry = function (entry, dm) {
            if (entry.ref)
                molmil.displayEntry(entry.ref, dm, true, this.soup);
            document.body.onmousedown();
        };
        ;
        UI.prototype.editLabel = function (settings) {
            var obj = null, text;
            var onOK = null;
            if (settings instanceof molmil.labelObject) {
                ...
                onOK = function (infoBox) {
                    ...
                    settings.dx = parseFloat(infoBox.dx.value) || 0.0;
                    settings.dy = parseFloat(infoBox.dy.value) || 0.0;
                    settings.dz = parseFloat(infoBox.dz.value) || 0.0;
                ...
                text = settings.text;
                settings.dx = settings.dx || 0.0;
                settings.dy = settings.dy || 0.0;
                settings.dz = settings.dz || 0.0;
        ...
        ;
        UI.prototype.showContextMenuAtom = function (x, y, pageX) {
        ...
        ;
        UI.prototype.showRM = function (icon, reset) {
                ...
                if (file instanceof molmil.entryObject) {
                    ...
                    item.plus.onclick = item.name.onclick = function (ev) {
                        if (ev.ctrlKey || ev.metaKey) {
                            ...
                            molmil_dep.expandCatTree.apply(this, []);
                    };
        ...
        ;
        UI.prototype.showChains = function (target, payload) {
            var chain, item;
            ...
            for (var i = 0; i < payload.chains.length; i++) {
                ...
                if (chain.authName && chain.authName != saa)
                    saa += " (" + chain.authName + ")";
        ...
        ;
        UI.prototype.showResidues = function (target, payload) {
            ...
            for (var i = 0; i < payload.molecules.length; i++) {
                ...
                item.payload = mol;
                item.UI = this.UI;
                if (this.UI.molSelection.indexOf(mol) != -1)
                ...
                item.onclick = function (e, passThrough) {
                    if (e.detail > 1)
                        return;
                    if (passThrough && this.UI.molSelection.indexOf(this.payload) != -1)
                        return;
                    if (e.altKey) {
                        var idx = this.UI.canvas.molmilViewer.atomSelection.indexOf(this.payload.CA || this.payload.atoms[0]);
                        if (idx == -1)
                            this.UI.canvas.molmilViewer.atomSelection.push(this.payload.CA || this.payload.atoms[0]);
                        else
                            this.UI.canvas.molmilViewer.atomSelection.splice(idx, 1);
                        var idx = this.UI.molSelection.indexOf(this.payload);
                        if (idx == -1) {
                            this.UI.molSelection.push(this.payload);
                            this.style.fontWeight = "bold";
                        }
                        else {
                            this.UI.molSelection.splice(idx, 1);
                        ...
                        if (this.UI.molSelection.length) {
                            ...
                            for (i = lastMID; i <= currMID; i++) {
                                ...
                                if (!tmp || this.UI.molSelection.indexOf(tmp.payload) != -1)
                                    continue;
                        ...
                        if (this.UI.molSelection.length == 1 && this.UI.molSelection[0] == this.payload) {
                            ...
                            for (var i = 0; i < this.UI.molSelection.length; i++)
                                this.UI.molRef[this.UI.molSelection[i].MID].style.fontWeight = "";
                            this.UI.molSelection = [this.payload];
                            this.style.fontWeight = "bold";
                        }
                    }
        ...
        //molmil.UI.prototype.
        UI.prototype.showLM = function (icon) {
            var UI = this;
            if (molmil.VRstatus === void 0)
                return molmil.initVR(null, function () { UI.showLM(icon); });
            try {
            ...
            e = menu.appendChild(document.createElement("div"));
            e.menu = menu;
            e.UI = this;
        ...
        ;
        UI.prototype.view = function (sub) {
            molmil_dep.Clear(sub);
            var e, UI = this;
            sub.style.display = "";
            e = sub.pushNode("div", "Reset zoom", "molmil_UI_ME");
            e.onclick = function () {
                if (UI.LM && UI.LM.parentNode.childNodes.length > 1)
                    UI.LM.onclick();
        ...
        UI.prototype.toggleWaters = function (show) {
        ...
        ;
        UI.prototype.toggleHydrogens = function (show) {
        ...
        ;
        UI.prototype.animationPopUp = function () {
            ...
            inp = label.pushNode("input");
            inp.type = "radio";
            inp.name = "motion";
            if (this.soup.animation.motionMode == 1)
                inp.checked = true;
            ...
            inp.type = "radio";
            inp.name = "motion";
            if (this.soup.animation.motionMode == 2)
                inp.checked = true;
            ...
            inp.type = "radio";
            inp.name = "motion";
            if (this.soup.animation.motionMode == 3)
                inp.checked = true;
            label.pushNode("span", "Swing");
            label.AI = this.soup.animation;
            label.onclick = function () { this.AI.motionMode = 3; };
            popup.pushNode("br");
            label = popup.pushNode("div");
            label.setClass("unselectableText");
            label.style.fontStyle = "normal";
            label.style.textAlign = "center";
            inp = label.pushNode("span", "|&#8920;");
            inp.setClass("smallButton");
            ...
            popup.close = popup.pushNode("button", "Close");
            popup.close.style.display = "block";
            popup.close.style.marginLeft = popup.close.style.marginRight = "auto";
        ...
        UI.prototype.resetRM = function () {
            try {
                molmil_dep.Clear(this.RM.parentNode.menu);
                if (this.RM.parentNode.menu.style.display != "none")
                    this.showRM(this.RM, true);
            }
            catch (e) { }
        };
        ;
        UI.prototype.toggleCLI = function () {
            if (this.LM && this.LM.parentNode.childNodes.length > 1)
                this.LM.onclick();
            if (!this.canvas.commandLine)
                new molmil.commandLine(this.canvas);
        ...
        UI.prototype.clear = function () {
            ...
            popup.yes = popup.pushNode("button", "Yes");
            popup.yes.canvas = this.canvas;
            popup.yes.UI = this;
            popup.yes.onclick = function () {
                if (this.canvas)
                    molmil.clear(this.canvas);
                this.UI.resetRM();
        ...
        ;
        UI.prototype.showDialog = function (func) {
            if (this.LM && this.LM.parentNode.childNodes.length > 1)
                this.LM.onclick();
            var popup = molmil_dep.dcE("div");
            popup.setClass("molmil_menu_popup");
            if (func)
                func.apply(this, [popup]);
            if (this.LM)
                this.LM.parentNode.pushNode(popup);
            return popup;
        };
        UI.prototype.xyz_input_popup = function (fp, fn, cb) {
        ...
        UI.prototype.ccp4_input_popup = function (fp, fn, cb) {
            this.showDialog(function (dialog) {
                var table = dialog.pushNode("table"), tr, td, load, cancel;
                ...
                load.onclick = function () {
                    var settings = {};
                    try {
                        settings.sigma = parseFloat(sigma.value);
                    }
                    catch (e) {
                        return alert("Incorrect value for sigma");
                    }
        ...
        UI.prototype.open = function (name, format, ondone, oncancel, binary) {
            if (this.LM && this.LM.parentNode.childNodes.length > 1)
                this.LM.onclick();
            if (this.onLMshow)
                this.onLMshow();
            ...
            popup.load.onclick = function () {
                ...
                r.onload = function (e) {
                    ...
                    if (this.fileFormat == "ccp4")
                        UI.ccp4_input_popup(e.target.result, this.filename, cb);
                    else if (this.fileFormat)
                        soup.loadStructureData(e.target.result, this.fileFormat, this.filename, cb, popup.settings);
                    else
                        ondone(this.filename, e.target.result);
                    oncancel = null;
                };
                if (binary)
                    r.readAsArrayBuffer(popup.inp.files[0]);
                else
                    r.readAsText(popup.inp.files[0]);
            };
            popup.cancel = popup.pushNode("button", "Cancel");
            var UI = this;
            popup.cancel.onclick = function () { this.popup.parentNode.removeChild(this.popup); if (UI.onLMhide)
                UI.onLMhide(); if (oncancel)
                oncancel(); };
            popup.cancel.popup = popup;
            if (this.LM)
                this.LM.parentNode.pushNode(popup);
        ...
        ;
        UI.prototype.openID = function (dbid) {
            if (this.LM && this.LM.parentNode.childNodes.length > 1)
                this.LM.onclick();
            if (this.onLMshow)
                this.onLMshow();
        ...
        UI.prototype.savePNG = function () {
            ...
            popup.save = popup.pushNode("button", "Save");
            popup.range.canvas = popup.save.canvas = this.canvas;
            popup.save.onclick = function () {
                if (!window.saveAs)
                    return molmil.loadPlugin(molmil.settings.src + "lib/FileSaver.js", this.onclick, this, []);
                if (molmil.configBox.stereoMode != 1 && !molmil.configBox.keepBackgroundColor) {
                    var opacity = molmil.configBox.BGCOLOR[3];
                    molmil.configBox.BGCOLOR[3] = 0;
                }
                ...
                if (mult != 1) {
                    ...
                    this.canvas.width *= mult;
                    this.canvas.height *= mult;
        ...
        UI.prototype.videoRenderer = function (justStart) {
            if (this.LM && this.LM.parentNode.childNodes.length > 1)
                this.LM.onclick();
            if (this.onLMshow)
                this.onLMshow();
            ...
            this.canvas.renderer.onRenderFinish = function () {
                if (!videoID)
                    return;
                if (window.addFrameCanvas)
                    addFrameCanvas(this.canvas);
                else if (window.addFrame)
                    addFrame(this.canvas.toDataURL());
                else {
            ...
            popup.endVideo = function () {
                if (this.canvas.molmilViewer.animation.playing)
                    molmil_dep.asyncStart(this.endVideo, [], this, 250);
                else {
                    this.canvas.renderer.selectDefaultContext();
                    molmil.settings.recordingMode = false;
                    if (window.finalizeVideo)
                        finalizeVideo();
                    else {
                        var req = new molmil_dep.CallRemote("GET");
                        req.AddParameter("id", videoID);
                        req.Send(molmil.settings.molmil_video_url + "deInitVideo");
                        if (molmil.settings.systemName)
                            window.location = molmil.settings.molmil_video_url + "getVideo?id=" + videoID + "&filename=" + molmil.settings.systemName + ".mp4";
                        else
                            window.location = molmil.settings.molmil_video_url + "getVideo?id=" + videoID;
                        popup.cancel.onclick();
                    }
                    videoID = null;
                }
            ...
            popup.record.onclick = function () {
                ...
                if (window.initVideo) {
                    videoID = 1;
                    molmil.initVideo(molmil.configBox.video_path, this.canvas.width, this.canvas.height, molmil.configBox.video_framerate);
                }
                else {
                    var req = new molmil_dep.CallRemote("GET");
            ...
            this.LM.parentNode.pushNode(popup);
            if (justStart)
                popup.record.onclick();
        };
        // new UI:
        //  - code wise, better structured
        //  - slide-based menu (no hierarchal display)
        UI.prototype.meshOptionsFunction = function (payload, lv, mode) {
            if (!(payload instanceof Array))
                payload = [payload];
            var mesh = payload[0], UI = this;
            if (!(mesh instanceof molmil.polygonObject))
                mesh = mesh.structures[0];
            UI.styleif_mesh(mesh, { clientX: window.innerWidth * .25, clientY: window.innerHeight * .25 }, {
                ondelete: function () {
        ...
        ;
        UI.prototype.deleteMeshFunction = function (payload, lv, mode) {
            if (!(payload instanceof Array))
                payload = [payload];
            var tmp = [];
            for (var f = 0; f < payload.length; f++) {
                if (payload[f].structures === undefined)
                    tmp.push(payload[f]);
                else {
                    for (var s = 0; s < payload[f].structures.length; s++)
                        if (payload[f].structures[s] instanceof molmil.polygonObject) {
            ...
            for (var f = 0; f < payload.length; f++) {
                ...
                for (var p = 0; p < pnames.length; p++)
                    this.soup.renderer.removeProgram(payload[f].programs[pnames[p]]);
                var idx = files.indexOf(payload[f]);
                if (idx != -1)
                    this.soup.structures.splice(idx, 1);
                else {
        ...
        UI.prototype.displayFunction = function (payload, lv, mode) {
            if (!(payload instanceof Array))
                payload = [payload];
            var tmp = [];
            for (var f = 0; f < payload.length; f++) {
            ...
            if (mode == 10001) {
                ...
                if (lv == 2) {
                    for (var i = 0; i < payload.length; i++) {
                        ...
                        for (var j = 0; j < payload[i].chain.entry.chains.length; j++)
                            if (tmp2.indexOf(payload[i].chain.entry.chains[j]) == -1)
                                tmp2.push(payload[i].chain.entry.chains[j]);
                    }
                }
                else if (lv == 3) {
                    for (var i = 0; i < payload.length; i++) {
                    ...
                    for (var i = 0; i < payload.length; i++) {
                        for (var _i = 0, _a = payload[i].chains; _i < _a.length; _i++) {
                            var chain = _a[_i];
                            tmp1.push(chain);
                            for (var j = 0; j < chain.entry.chains.length; j++)
                                if (tmp2.indexOf(chain.entry.chains[j]) == -1)
                                    tmp2.push(chain.entry.chains[j]);
                        }
                    }
                }
                molmil.calcHbonds(tmp1, tmp2, { type: "dotted-cylinder", rgba: [0, 0, 0, 255] }, this.soup);
                var tmp = [];
            }
            else if (mode == 10002) {
            ...
            for (var i = 0; i < payload.length; i++) {
                if (payload[i] instanceof molmil.atomObject) {
                    if (lv == 2)
                        payload[i] = payload[i].molecule;
                    if (lv == 3)
                        payload[i] = payload[i].chain;
                    if (lv == 4)
                        payload[i] = payload[i].chain.entry;
                }
            }
        ...
        UI.prototype.popupMenuBuilder = function (title, fields, ondone, oncancel) {
            var popup = molmil_dep.dcE("div");
            popup.className = "molmil_popup";
            ...
            for (i = 0; i < fields.length; i++) {
                ...
                if (fields[i][2] == "color") {
                    tmp = field.valueBox.pushNode("input");
                    tmp.type = "color";
                    if (fields[i][3])
                        tmp.value = molmil.rgb2hex(fields[i][3][0], fields[i][3][1], fields[i][3][2]);
                }
                    ...
                    tmp = field.valueBox.pushNode("input", fields[i][3] || "");
                    tmp.type = "text";
                    tmp.value = fields[i][3] || "";
                }
                popup.objects[fields[i][1]] = tmp;
            }
            var OK = popup.pushNode("button", "OK");
            OK.popup = popup;
            OK.onclick = function () { this.popup.ondone(this.popup.objects); this.popup.destroy(); };
            var cancel = popup.pushNode("button", "Cancel");
            cancel.popup = popup;
            cancel.onclick = function () { if (this.popup.oncancel) {
                this.popup.oncancel(this.popup.objects);
            } this.popup.destroy(); };
            document.documentElement.pushNode(popup);
        };
        ;
        UI.prototype.colorFunction = function (payload, lv, mode, setting) {
            if (mode == molmil.colorEntry_Custom && !setting) {
                ...
                return this.popupMenuBuilder("Color object", [["Color by:", "color", "color"], ["Color cartoon", "colorCartoon", "checkbox", true]], function (objects) {
                    var rgba = molmil.hex2rgb(objects.color.value);
                    rgba.push(255);
        ...
        ;
        UI.prototype.labelFunction = function (payload, lv) {
            if (!(payload instanceof Array))
                payload = [payload];
            var objs = [], text = "";
            if (lv == 0) { // hide
            ...
            if (lv == 1000) { // show
                payload[0].display = true;
                payload[0].status = false;
                this.soup.canvas.update = true;
                return;
            }
            if (lv == 0.5) {
            ...
            if (lv == 1) { // atom
                ...
                if (objs.length == 1)
                    ...
                    text = objs.length + " atoms";
            }
            else if (lv == 2) { // residue
                for (var i = 0; i < payload.length; i++) {
                    if (payload[i] instanceof molmil.atomObject)
                        objs.push(payload[i].molecule);
                    if (payload[i] instanceof molmil.molObject)
                        objs.push(payload[i]);
                }
                if (objs.length == 1)
                    text = objs[0].toString();
                else
                    text = objs.length + " residues";
            }
            else if (lv == 3) { // chain
                for (var i = 0; i < payload.length; i++) {
                    if (payload[i] instanceof molmil.atomObject)
                        objs.push(payload[i].chain);
                    if (payload[i] instanceof molmil.chainObject)
                        objs.push(payload[i]);
                }
                if (objs.length == 1)
                    text = objs[0].toString();
                else
                    text = objs.length + " chains";
            }
            else if (lv == 4) { // entry
                for (var i = 0; i < payload.length; i++) {
                    if (payload[i] instanceof molmil.atomObject)
                        objs.push(payload[i].chain.entry);
                    if (payload[i] instanceof molmil.entryObject)
                        objs.push(payload[i]);
                }
                if (objs.length == 1)
                    text = objs[0].toString();
                else
                    text = objs.length + " entries";
            }
                ...
                ["Display", 0, [
                        ["Residue", 0, [
                                [function (payload, structure) {
                                        if (payload[0].molecule.display) {
                                            structure[3][2] = molmil.displayMode_None;
                                            return "Hidden";
                                        }
                                        else {
                                            structure[3][2] = molmil.displayMode_Visible;
                                            return "Visible";
                                        }
        ...
        UI.prototype.buildComplexMenu = function (title, structure, previousCall, payload) {
            ...
            if (!previousCall) {
                ...
                if (mw < this.complexMenu.position[2] + (molmil_dep.fontSize * 15))
                    delta = (this.complexMenu.position[2] + (molmil_dep.fontSize * 15)) - mw;
            ...
            if (title)
                this.complexMenu.titleDIV.innerHTML = title;
            // each item contains:
            //  - title for the item
            ...
            for (i = 0; i < structure.length; i++) {
                ...
                item.UI = this;
                item.structure = structure[i];
                if (typeof structure[i][0] == "function")
                    item.text = item.pushNode("span", structure[i][0].apply(this, [payload, structure[i]])); // if structure[i][0] is a function -> execute this function
                else
                    item.text = item.pushNode("span", structure[i][0]);
                item.button = item.pushNode("span", structure[i][1] ? "&nbsp;" : ">");
                item.onclick = function () {
                    if (!this.structure[1])
                        this.UI.buildComplexMenu((this.UI.complexMenu.titleDIV.innerHTML ? this.UI.complexMenu.titleDIV.innerHTML + " > " : "") + this.structure[0], this.structure[2], [title, structure, previousCall, payload], payload);
                    else {
        ...
        ;
        UI.prototype.styleif_au = function (contentBox) {
            var UI = this;
            ...
            tmp = styleSelect.pushNode("option", "Style");
            tmp.disabled = true;
            tmp.value = "placeholder";
            tmp.style.display = "none";
            tmp = styleSelect.pushNode("option", "Cartoon");
            tmp.value = molmil.displayMode_Default;
            tmp = styleSelect.pushNode("option", "Cartoon with sidechains");
            tmp.value = molmil.displayMode_Stick_SC;
            tmp = styleSelect.pushNode("option", "Sticks");
            tmp.value = molmil.displayMode_Stick;
            tmp = styleSelect.pushNode("option", "Wireframe");
            tmp.value = molmil.displayMode_Wireframe;
            contentBox.pushNode("span", "&nbsp;");
            var colorSelect = contentBox.pushNode("select");
            tmp = colorSelect.pushNode("option", "Color");
            tmp.disabled = true;
            tmp.value = "placeholder";
            tmp.style.display = "none";
            tmp = colorSelect.pushNode("option", "Group");
            tmp.value = 0;
            tmp = colorSelect.pushNode("option", "Structure");
            tmp.value = 1;
            tmp = colorSelect.pushNode("option", "Chain");
            tmp.value = 2;
            tmp = colorSelect.pushNode("option", "Entity");
            tmp.value = 3;
            tmp = colorSelect.pushNode("option", "B-factor");
            tmp.value = 4;
            styleSelect.oninput = function () {
                if (styleSelect.value == "placeholder")
                    return;
                if (styleSelect.value == molmil.displayMode_Stick_SC)
                    molmil.displayEntry(UI.soup.structures, molmil.displayMode_Default, false, UI.soup, { newweb: true });
            ...
            colorSelect.oninput = function () {
                if (colorSelect.value == "placeholder")
                    return;
                if (colorSelect.value == 1)
                    molmil.colorEntry(UI.soup.structures, molmil.colorEntry_Default, null, true, UI.soup);
                else if (colorSelect.value == 2)
                    molmil.colorEntry(UI.soup.structures, molmil.colorEntry_ChainAlt, { carbonOnly: true }, true, UI.soup);
                else if (colorSelect.value == 3)
                    molmil.colorEntry(UI.soup.structures, molmil.colorEntry_Entity, { carbonOnly: true }, true, UI.soup);
                    ...
                    if (molmil.configBox.bfactor_low != undefined)
                        ...
                        var min = molmil.arrayMin(values);
                    if (molmil.configBox.bfactor_high != undefined)
                        var max = molmil.configBox.bfactor_high;
                    else
                        var max = molmil.arrayMax(values);
                    var diffInv = 1. / (max - min), tmp;
                    for (var i = 0; i < selection.length; i++) {
                        tmp = 1 - ((values[i] - min) * diffInv); ///TODO
                        selection[i].rgba = molmil.hslToRgb123(tmp * (2 / 3), 1.0, 0.5);
                        selection[i].rgba[0] *= 255;
                        selection[i].rgba[1] *= 255;
                        selection[i].rgba[2] *= 255;
                        selection[i].rgba.push(255);
        ...
        ;
        UI.prototype.styleif_bu = function (contentBox, afterDL) {
            var UI = this;
            ...
            if (afterDL)
                delete this.soup.downloadInProgress--;
            ...
            if (!this.soup.sceneBU) {
                ...
                if (sceneBU.NOC > 1 && sceneBU.isBU && (sceneBU.type == 2 || sceneBU.size > 30000))
                    sceneBU.displayMode = 5;
                else
                    sceneBU.displayMode = 3;
                sceneBU.colorMode = 2;
            ...
            if (struct.meta.pdbxData.struct_asym && struct.meta.pdbxData.struct_asym && struct.meta.pdbxData.entity) {
                for (c = 0; c < struct.meta.pdbxData.struct_asym.entity_id.length; c++)
                    asym2mw[struct.meta.pdbxData.struct_asym.id[c]] = struct.meta.pdbxData.entity.formula_weight[struct.meta.pdbxData.entity.id.indexOf(struct.meta.pdbxData.struct_asym.entity_id[c])];
            }
            for (c = 0; c < soup.chains.length; c++) {
                if (soup.poly_asym_ids.indexOf(soup.chains[c].name) != -1)
                    noc++;
                MW += asym2mw[soup.chains[c].name] || 0;
            }
            td = US.pushNode("option", "Asymmetric unit (%NOC chains, %D D)".replace("%NOC", noc).replace("%D", molmil_dep.Rounding(MW, 2)));
            td.value = -1;
                ...
                for (var e in struct.BUassemblies) {
                    ...
                    for (i = 0; i < struct.BUassemblies[e].length; i++) {
                        ...
                        for (c = 0; c < struct.BUassemblies[e][i][1].length; c++) {
                            if (soup.poly_asym_ids.indexOf(struct.BUassemblies[e][i][1][c]) != -1)
                                tmp++;
                    ...
                    td = US.pushNode("option", txt);
                    td.value = e;
                }
                if (assembly && (assembly == -1 || struct.BUassemblies.hasOwnProperty(assembly)))
                    assembly_id = assembly;
                else {
                    assembly = figureOutAssemblyId(soup.pdbxData, struct.BUassemblies);
                    if (struct.BUassemblies.hasOwnProperty(assembly))
                        assembly_id = assembly;
                }
            }
            ...
            td = BUrm.pushNode("option", "Only backbone, colored by CPK");
            td.value = [1, 1];
            td = BUrm.pushNode("option", "Only backbone, colored by each asymmetric chain");
            td.value = [1, 2];
            td = BUrm.pushNode("option", "Only backbone, colored by each chain");
            td.value = [1, 3];
            td = BUrm.pushNode("option", "Only alpha carbon and phosphorus, colored by CPK");
            td.value = [2, 1];
            td = BUrm.pushNode("option", "Only alpha carbon and phosphorus, colored by each asymmetric chain");
            td.value = [2, 2];
            td = BUrm.pushNode("option", "Only alpha carbon and phosphorus, colored by each chain");
            td.value = [2, 3];
            td = BUrm.pushNode("option", "Tube, colored by each asymmetric chain");
            td.value = [3, 2];
            td = BUrm.pushNode("option", "Tube, colored by each chain");
            td.value = [3, 3];
            td = BUrm.pushNode("option", "Tube, colored by secondary structure");
            td.value = [3, 4];
            td = BUrm.pushNode("option", "Cartoon, colored by each asymmetric chain");
            td.value = [4, 2];
            td = BUrm.pushNode("option", "Cartoon, colored by each chain");
            td.value = [4, 3];
            td = BUrm.pushNode("option", "Cartoon, colored by secondary structure");
            td.value = [4, 4];
            td = BUrm.pushNode("option", "Rocket, colored by each asymmetric chain");
            td.value = [6, 2];
            td = BUrm.pushNode("option", "Rocket, colored by each chain");
            td.value = [6, 3];
            td = BUrm.pushNode("option", "Rocket, colored by secondary structure");
            td.value = [6, 4];
            td = BUrm.pushNode("option", "Coarse surface, colored by each asymmetric chain");
            td.value = [5, 2];
            td = BUrm.pushNode("option", "Coarse surface, colored by each chain");
            td.value = [5, 3];
            BUrm.value = rm;
        ...
        ;
        UI.prototype.styleif_cc = function (contentBox, afterDL) {
            var UI = this;
            ...
            opt.onclick = function () {
                if (labels === undefined) {
                    ...
                    for (var i = 0; i < labels.length; i++) {
                        labels[i].display = true;
                        labels[i].status = false;
                    }
                }
                if (this.innerHTML == "Show atom labels") {
                    for (var i = 0; i < labels.length; i++) {
                        if (labels[i].settings.atomSelection[0].element == "H" && !UI.soup.showHydrogens)
                            labels[i].display = false;
                        else
                            labels[i].display = true;
                        labels[i].status = false;
                    }
                    ...
                    for (var i = 0; i < labels.length; i++)
                        labels[i].display = labels[i].status = false;
        ...
        ;
        UI.prototype.drag_panel = function (title, top, left) {
        ...
        UI.prototype.styleif_mesh = function (mesh, ev, options) {
            var UI = this;
            var fname = options.filename || mesh.meta.filename;
            if (document.getElementById("mesh_options_for_" + fname))
                return;
            ...
            if (settings.solid)
                solid_mode.checked = true;
            item.onclick = function () {
                if (settings.solid)
                    return;
                if (!("alphaSet" in settings))
                    settings.alphaSet = 1.0;
                mesh.programs[0].toggleWF();
                UI.soup.renderer.canvas.update = true;
            ...
            if (settings.rgba) {
                ...
                item.oninput = function () {
                    ...
                    settings.rgba[0] = rgb[0];
                    settings.rgba[1] = rgb[1];
                    settings.rgba[2] = rgb[2];
                    UI.soup.renderer.canvas.update = true;
                };
            }
            cont = div.pushNode("div");
            ...
            transp.oninput = function () {
                ...
                if (mesh.programs[0].settings.alphaSet != 1 && mesh.programs[0].pre_shader != UI.soup.renderer.shaders.alpha_dummy)
                    mesh.programs[0].rebuild();
                UI.soup.renderer.canvas.update = true;
            };
            cont.pushNode("br");
        ...
        UI.prototype.styleif_edmap = function (contentBox, callOptions) {
            ...
            if (struct.meta.styleif === undefined)
                return setTimeout(function () { UI.styleif_edmap(contentBox, callOptions); }, 100);
            ...
            var edmap2_downloads = function () {
                ...
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>Structure factors</a>';
                idx = nwfls.type.indexOf("2fo-fc-cif");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>2fo-fc map (cif)</a>';
                idx = nwfls.type.indexOf("fo-fc-cif");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>fo-fc map (cif)</a>';
                idx = nwfls.type.indexOf("2fo-fc-mtz");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>2fo-fc map (mtz)</a>';
                idx = nwfls.type.indexOf("fo-fc-mtz");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>fo-fc map (mtz)</a>';
                idx = nwfls.type.indexOf("2fo-fc-ccp4");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>2fo-fc map (ccp4)</a>';
                idx = nwfls.type.indexOf("fo-fc-ccp4");
                if (idx != -1)
                    content += '<a href="' + molmil.settings.data_url + nwfls.name[idx] + '" download>fo-fc map (ccp4)</a>';
            ...
            form.mode = "wwpdb";
            if (mode == "2fo-fc" || mode == "fo-fc")
                form.mode = mode;
            var cont, item, downloads = flexCont.pushNode("div");
            downloads.id = "nw_edmap_dl";
        ...
        ;
        UI.prototype.styleif_sites = function (contentBox) {
            var UI = this;
            var mmjsonGroupData = function (obj, field) {
                ...
                for (var i = 0, k; i < obj[field].length; i++) {
                    if (!(obj[field][i] in output))
                        output[obj[field][i]] = [];
                    tmp = {};
                    for (k = 0; k < keys.length; k++)
                        tmp[keys[k]] = obj[keys[k]][i];
            ...
            for (var s = 0; s < this.soup.structures.length; s++) {
                ...
                if (!struct && !struct.meta || !struct.meta.pdbid)
                    continue;
                if (struct.meta.styleif === undefined)
                    return setTimeout(function () { UI.styleif_sites(contentBox); }, 100);
            }
            var downloads = contentBox.pushNode("div");
            downloads.id = "nw_edmap_dl";
            for (var i = 0; i < this.soup.structures.length; i++) {
                if (!this.soup.structures[i].meta || !this.soup.structures[i].meta.filename || !this.soup.structures[i].meta.filename.endsWith(".mpbf"))
                    continue;
                var cont = downloads.pushNode("div");
            ...
            for (var s = 0; s < this.soup.structures.length; s++) {
                ...
                if (!struct.meta.styleif.fgroups) {
                    var struct_site_pdbmlplus = struct.meta.styleif.struct_site_pdbmlplus || { info_subtype: [] }, struct_site_gen_pdbmlplus = struct.meta.styleif.struct_site_gen_pdbmlplus, entity_poly = struct.meta.pdbxData.entity_poly || struct.meta.styleif.entity_poly, struct_site = struct.meta.pdbxData.struct_site || struct.meta.styleif.struct_site || { id: [] }, struct_site_gen = struct.meta.pdbxData.struct_site_gen || struct.meta.styleif.struct_site_gen;
                    var buffer = {};
                    for (var e in struct_site_pdbmlplus)
                        buffer[e] = struct_site_pdbmlplus[e].map(function (x) { return x; });
                    ...
                    for (i = 0; i < buffer.id.length; i++) {
                        ...
                        if (buffer.info_subtype[i] == "binding" || buffer.info_subtype[i] == "prosite") {
                            ...
                            for (r = 0; r < residues.length; r++) {
                                ...
                                try {
                                    if (tmp2[0] == null)
                                        tmp2 = [UI.soup.getMolObject4ChainAlt(chains, residx).RSID, UI.soup.getMolObject4ChainAlt(chains, residx2).RSID];
                                }
                                catch (e) { }
                                if (tmp2[0] != null)
                                    tmp.push([chainid + ": " + tmp2[0] + (tmp2[0] != tmp2[1] ? "-" + tmp2[1] : "")]);
                            }
                        }
                        fgroup.name = tmp.join(", ") + (buffer.info_subtype[i] && buffer.info_subtype[i] != "pdb" ? " (" + buffer.info_subtype[i] + ")" : "");
                        if (fgroup.residueList.length)
                            fgroups.push(fgroup);
                    }
                }
                else
                    var fgroups = struct.meta.styleif.fgroups;
        ...
        ;
        UI.prototype.styleif_align = function (contentBox) {
            var UI = this;
            var alignments = [];
            contentBox.id = "nw_align";
            for (var i in molmil.alignInfo) {
                if (!UI.soup.chains.includes(molmil.alignInfo[i].chain1) || !UI.soup.chains.includes(molmil.alignInfo[i].chain2))
                    continue;
            ...
            var show = function (aid) {
                ...
                while (true) {
                    fpos = pos + nlen;
                    if (fpos >= alignment.alignment.length)
                        fpos = alignment.alignment.length - 1;
                    ...
                    c1pos += Array.from(alignment.seq1.substring(pos, fpos)).filter(function (x) { return x != "-"; }).length;
                    ...
                    c2pos += Array.from(alignment.seq2.substring(pos, fpos)).filter(function (x) { return x != "-"; }).length;
                    tr.pushNode("td", alignment.chain2.molecules[c2pos - 1].RSID);
                    pos += nlen;
                    if (pos >= alignment.alignment.length)
                        break;
                }
            };
            show(0);
        ...
        ;
        UI.prototype.styleif_settings = function (contentBox) {
            /*
            ...
            */
            ...
            var easySet = function (what, to) {
                if (localStorage.getItem(what) == to)
                    return;
            ...
            var applyUpdate = function () {
                ...
                if (easySet("molmil.settings_glsl_fog", (molmil.configBox.glsl_fog = saveContainer.fog.checked) ? "1" : "0")) {
                    recompile = true;
                    if (saveContainer.fog.checked)
                        saveContainer.slab.toggle.checked = UI.soup.renderer.settings.slab = false;
                }
                ...
                if (easySet("molmil.settings_slab_near_ratio", saveContainer.slab.near.value))
                    slab = true;
                if (easySet("molmil.settings_slab_far_ratio", saveContainer.slab.far.value))
                    slab = true;
                // reload settings
                molmil.initSettings();
                UI.soup.reloadSettings();
                molmil.configBox.projectionMode = saveContainer.projectionMode.value;
                molmil.configBox.stereoMode = parseInt(saveContainer.stereoMode.value);
                ...
                if (resizeVP)
                    UI.soup.renderer.resizeViewPort();
                if (recompile)
                    molmil.shaderEngine.recompile(UI.soup.renderer);
                if (slab) {
            ...
            saveContainer.qlv.type = "range";
            saveContainer.qlv.min = "0";
            saveContainer.qlv.max = "4";
            saveContainer.qlv.step = "1";
            saveContainer.qlv.value = localStorage.getItem("molmil.settings_QLV");
            saveContainer.qlv.ref = molmil_dep.dcE("span");
            saveContainer.qlv.ref.innerHTML = saveContainer.qlv.value;
            ...
            saveContainer.bbsf.type = "range";
            saveContainer.bbsf.min = "0";
            saveContainer.bbsf.max = "4";
            saveContainer.bbsf.step = "1";
            saveContainer.bbsf.value = molmil.configBox.smoothFactor;
            saveContainer.bbsf.ref = molmil_dep.dcE("span");
            saveContainer.bbsf.ref.innerHTML = saveContainer.bbsf.value;
            ...
            tmp = saveContainer.projectionMode.pushNode("option", "Perspective projection");
            tmp.value = 1;
            tmp = saveContainer.projectionMode.pushNode("option", "Orthographic projection");
            tmp.value = 2;
            ...
            tmp = saveContainer.stereoMode.pushNode("option", "None");
            tmp.value = 0;
            tmp = saveContainer.stereoMode.pushNode("option", "Anaglyph");
            tmp.value = 1;
            if (molmil.configBox.stereoMode == 1)
                tmp.selected = true;
            tmp = saveContainer.stereoMode.pushNode("option", "Side-by-side");
            tmp.value = 2;
            if (molmil.configBox.stereoMode == 2)
                tmp.selected = true;
            tmp = saveContainer.stereoMode.pushNode("option", "Cross-eyed");
            tmp.value = 4;
            if (molmil.configBox.stereoMode == 4)
                tmp.selected = true;
            ...
            saveContainer.bgcolor.A.min = 0;
            saveContainer.bgcolor.A.max = 255;
            saveContainer.bgcolor.A.step = 1;
            saveContainer.bgcolor.A.value = molmil.configBox.BGCOLOR[3] * 255;
            saveContainer.bgcolor.A.ref = saveContainer.bgcolor.pushNode("span");
            saveContainer.bgcolor.A.ref.innerHTML = saveContainer.bgcolor.A.value;
        ...
        ;
        UI.prototype.styleif = function (showOption, callOptions) {
            var UI = this;
            var initStruct = function (struct) {
                if (!struct.meta.id || !struct.meta.pdbid)
                    return;
                var pdbid = struct.meta.pdbid.toLowerCase();
                if (struct.meta.styleif === undefined) {
                    ...
                    request.OnDone = function () {
                        ...
                        if (tmp1.info_subtype.length || tmp2.id.length)
                            UI.showSites = true;
                    };
            ...
            for (var i = 0; i < UI.soup.structures.length; i++)
                initStruct(UI.soup.structures[i]);
            ...
            if (!nwif) {
                ...
                if (window.MutationObserver !== undefined) {
                    ...
                    if (styleif_height == "hidden")
                        showOption = "hide";
                    var observer = new MutationObserver(function (mutations) {
                        if (nwif.contentBox.classList.contains("visible"))
                            localStorage.setItem("molmil.settings_styleif_height", nwif.contentBox.style.height);
                        else
                            localStorage.setItem("molmil.settings_styleif_height", "hidden");
                    });
                ...
                var options = [
                    ...
                    ["Chemical compontent", "cc", function () {
                            return UI.soup.pdbxData && UI.soup.pdbxData.chem_comp_atom && UI.soup.pdbxData.struct
                                === undefined;
                        }, function () { UI.styleif_cc(nwif.contentBox); }],
                ...
                if (molmil.configBox.menuOptions && molmil.configBox.menuOptions.length)
                    options = options.concat(molmil.configBox.menuOptions);
                var doHandler = function (ev, callOptions) {
                    ...
                    if (!nwif.contentBox.classList.contains("visible")) {
                        if (nwif.firstChild.style.height)
                            nwif.heightSet = nwif.firstChild.style.height;
                        nwif.firstChild.style.height = "";
                    }
                    else if (nwif.heightSet)
                        nwif.firstChild.style.height = nwif.heightSet;
                    nwif.options.classList.remove("visible");
                };
                for (var i = 0; i < options.length; i++) {
                    var opt = nwif.options.pushNode("div", options[i][0]);
                    if (options[i][2])
                        opt.checkHandler = options[i][2];
                    else
                        opt.checkHandler = function () { return true; };
                    opt.value = options[i][1];
                    opt.onclick = doHandler;
                }
                nwif.button.onclick = function () {
                    for (var i = 0; i < nwif.options.childNodes.length; i++)
                        nwif.options.childNodes[i].style.display = nwif.options.childNodes[i].checkHandler() ? "" : "none";
                    nwif.options.classList.toggle("visible");
                };
            }
            if (showOption == "bu" && UI.soup.AisB)
                showOption = "structure";
            for (var i = 0; i < nwif.options.childNodes.length; i++) {
                var opt = nwif.options.childNodes[i];
                if (opt.value == showOption)
                    opt.onclick(null, callOptions);
            }
        };
        ;
        return UI;
...
var molmil;
(function (molmil) {
    var animationObj = /** @class */ (function () {
        function animationObj(soup) {
            this.soup = soup;
        ...
        ;
        animationObj.prototype.updateInfoBox = function () {
            if (!this.infoBox)
                return;
            if (this.infoBox.timeBox) {
                if (this.soup.frameInfo)
                    this.infoBox.timeBox.innerHTML = this.soup.frameInfo[this.frameNo][1].toFixed(1) + " ps (" + (this.frameNo + 1) + ")";
                else
                    this.infoBox.timeBox.innerHTML = this.frameNo;
            }
            if (this.infoBox.sliderBox) {
                if (this.soup.frameInfo)
                    this.infoBox.timeBox.value = this.soup.frameInfo[this.frameNo][1];
                else
                    this.infoBox.timeBox.value = this.frameNo;
                this.infoBox.sliderBox.value = this.frameNo;
            }
        };
        ;
        animationObj.prototype.beginning = function () {
            if (!this.init)
                this.initialise();
            this.frameNo = 0;
        ...
        ;
        animationObj.prototype.go2Frame = function (fid) {
            if (!this.init)
                this.initialise();
            this.frameNo = fid;
            this.renderer.selectFrame(this.frameNo, this.detail_or);
        ...
        animationObj.prototype.previous = function () {
            if (!this.init)
                this.initialise();
            this.frameNo -= 1;
            if (this.frameNo < 0)
        ...
        ;
        animationObj.prototype.pause = function () {
            if (this.TID) {
        ...
        ;
        animationObj.prototype.play = function () {
            if (!this.init)
                this.initialise();
            this.playing = true;
        ...
        ;
        animationObj.prototype.next = function () {
            if (!this.init)
                this.initialise();
            this.frameNo += 1;
        ...
        ;
        animationObj.prototype.end = function () {
            if (!this.init)
                this.initialise();
        ...
        ;
        animationObj.prototype.forwardRenderer = function () {
            if (this.number_of_frames < 2)
                return;
            this.frameNo += 1;
            if (this.frameNo >= this.number_of_frames) {
                if (this.motionMode == 3 || this.motionMode == 3.5) {
                    this.frameNo -= 1;
                    return this.backwardRenderer();
                }
                else
                    this.playing = false;
            }
        ...
        ;
        animationObj.prototype.backwardRenderer = function () {
            if (this.number_of_frames < 2)
                return;
            this.frameNo -= 1;
            if (this.frameNo < 0) {
                if (this.motionMode == 3 && !molmil.settings.recordingMode) {
                    this.frameNo += 1;
...
(function (molmil) {
    ...
    molmil.setOnContextMenu = setOnContextMenu;
    ...
    function bindCanvasInputs(canvas) {
        ...
        var renderOnlyFinal = function (soup, structures) {
            ...
            if (Array.isArray(structures))
                nfilesproc[2] = nfilesproc[2].concat(structures);
            else
                nfilesproc[2].push(structures);
            if (nfilesproc[0] < nfilesproc[1])
                return;
        ...
        var pasteDB = function (ev) {
            if (ev.srcElement.className.includes("molmil_UI_cl_input"))
                return;
            ev.preventDefault();
        ...
        var processFiles = function (files, files2) {
            ...
            var bakacheck = function () {
                ...
                for (i = 0; i < count; i++) {
                    if (items[i].toLowerCase().endsWith(".mjs")) {
                        mjsFile = items[i];
                        break;
                    }
                }
                ...
                for (i = 0; i < count; i++) {
                    ...
                    for (j = 0; j < canvas.inputFunctions.length; j++) {
                        if (canvas.inputFunctions[j](canvas, fr)) {
                            ok = true;
                            break;
                        }
                    }
                    if (!ok) {
                        for (var j in molmil.formatList) {
                            if (typeof molmil.formatList[j] != "function" || !cmd[1].endsWith(j))
                                continue;
        ...
        canvas.inputFunctions.push(function (canvas, fr) {
            if (fr.filename.endsWith(".gz")) {
                ...
                fr.onload = function (e) {
                    if (!window.hasOwnProperty("pako"))
                        return molmil_dep.asyncStart(this.onload, [e], this, 50);
                    ...
                    for (j = 0; j < canvas.inputFunctions.length; j++) {
                        if (canvas.inputFunctions[j](canvas, fakeObj))
                            break;
                    }
                };
        ...
        canvas.inputFunctions.push(function (canvas, fr) {
            if (fr.filename.endsWith(".zip")) {
                if (!window.hasOwnProperty("unzip")) {
                    var obj = molmil_dep.dcE("script");
                    obj.src = molmil.settings.src + "lib/unzipit.min.js";
                    document.getElementsByTagName("head")[0].appendChild(obj);
                }
                fr.onload = function (e) {
                    if (!window.hasOwnProperty("unzipit"))
                        return molmil_dep.asyncStart(this.onload, [e], this, 50);
    ...
    function initVideo(UI) {
        ...
        if (molmil.settings.molmil_video_url === undefined && window.SharedArrayBuffer !== undefined) {
            ...
            var obj = molmil_dep.dcE("script");
            obj.src = molmil.settings.src + "lib/ffmpeg_handler.js";
